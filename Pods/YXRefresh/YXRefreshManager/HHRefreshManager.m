...
#define GetWidth(__A) (__A*([[UIScreen mainScreen] bounds].size.width)/375)
#define REFRESHHEIGHT GetWidth(60)
...
- (void)setIsHeaderNoMore:(BOOL)isHeaderNoMore{
    _isHeaderNoMore = isHeaderNoMore;
    self.headerView.isNoMore = isHeaderNoMore;
}

- (void)setIsFooterNoMore:(BOOL)isFooterNoMore{
    _isFooterNoMore = isFooterNoMore;
    self.footerView.isNoMore = _isFooterNoMore;
}

-(void)setNoMoreString:(NSString *)noMoreString{
    _noMoreString = noMoreString;
    self.headerView.noMoreString = noMoreString;
    self.footerView.noMoreString = noMoreString;
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context
{
    ...
    if (!self.isFooterNoMore){
        _footerView.y =_scrollView.height >_scrollView.contentSize.height?_scrollView.height:_scrollView.contentSize.height;
    } else {
        _footerView.y =_scrollView.contentSize.height;
    }
    ...
    if (_scrollView.isDragging) {
        ...
        if (_offsetY>0) {
            if (self.isFooterNoMore&&self.animationType == AnimationTypeNormal)return;
            ...
            if (self.isHeaderNoMore&&self.animationType == AnimationTypeNormal)return;
    ...
    if (type == HHRefreshTypeHeader) {
        ...
        if (_isHeaderRefresh) {
            ...
            [UIView animateWithDuration:ANIMTIONTIME animations:^{
                if (self.isFooterNoMore) {
                    [self.scrollView setContentInset:UIEdgeInsetsMake(0, 0, REFRESHHEIGHT, 0)];
                    self.footerView.y = self.scrollView.contentSize.height;
                }else{
                    [self.scrollView setContentInset:UIEdgeInsetsZero];
                }
            }];
