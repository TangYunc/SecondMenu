...
-(void)initialize{
    ...
    _currentDotSize = CGSizeMake(10, 3);
    _otherDotSize = CGSizeMake(5, 3);
}
...
-(void)createPointView{
    ...
    switch (_pageControlStyle) {
        ...
        {
            [self creatDotWithSize:self.currentDotSize withOtherDotSize:self.otherDotSize];
        }
            break;
        default:
            break;
...
-(void)creatDotWithSize:(CGSize)currentDotSize withOtherDotSize:(CGSize)otherDotSize{
    ...
    //居中控件
    CGFloat startX=0;
    CGFloat currentStartY=0;
    CGFloat otherStartY=0;
    ...
    if(self.frame.size.height<currentDotSizeH){
        currentStartY = 0;
        currentDotSizeH = self.frame.size.height;
        otherStartY = (currentDotSizeH-otherDotSizeH)/2;
    }else{
        currentStartY = self.frame.size.height-currentDotSizeH;
        otherStartY = currentStartY + (currentDotSizeH-otherDotSizeH)/2;
    }
    
    if (otherDotSizeH>currentDotSizeH) {
        otherDotSizeH = currentDotSizeH;
        otherStartY =currentStartY;
    }
    ...
    for (int page=0; page<_numberOfPages; page++) {
        if(page==_currentPage){
            UIView *currPointView=[[UIView alloc]initWithFrame:CGRectMake(startX, currentStartY, currentDotSizeW, currentDotSizeH)];
            ...
            UIView *otherPointView=[[UIView alloc]initWithFrame:CGRectMake(startX, otherStartY, otherDotSizeW, otherDotSizeH)];
            otherPointView.backgroundColor=_otherColor;
            otherPointView.tag=page+1000;
