...
@property (nonatomic, strong) NSArray *pageControlVConstraints;
@end
...
-(JhPageControl *)pageControl{
    if (!_pageControl) {
        ...
        JhPageControl *pageControl = [[JhPageControl alloc] init];//WithFrame:CGRectMake(0, 0, kScreenWidth-kGetWidth(12)*2, 10)];
        pageControl.translatesAutoresizingMaskIntoConstraints = NO;
        pageControl.currentPage = 1;
        pageControl.otherColor = RGB_COLOR_WITH_0x_alpha(WhiteColor, 0.5);
        pageControl.currentColor = RGB_COLOR_WITH_0x(WhiteColor);
        pageControl.pageControlContentMode = SDCycleScrollViewPageContolAlimentCenter; //设置对齐方式
        pageControl.controlSpacing = 4.0; //间距
        pageControl.pageControlStyle = JhPageControlStyelDotAndRectangle; //圆点 + 长条 样式
        _pageControl =pageControl;
        [self addSubview:_pageControl];
        
        NSArray * pageControlVConstraints = [NSLayoutConstraint constraintsWithVisualFormat:[NSString stringWithFormat:@"V:[pageControl(10)]-(%f)-|",self.pageControlBottom] options:0 metrics:nil views:@{@"pageControl":pageControl}];
        [self addConstraints:pageControlVConstraints];
        self.pageControlVConstraints = pageControlVConstraints;
        [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-0-[pageControl]-0-|" options:0 metrics:nil views:@{@"pageControl":pageControl}]];
    }
    return _pageControl;
}
...
-(void)setPageControlBottom:(CGFloat)pageControlBottom{
    ...
    if (self.pageControlVConstraints) {
        [self removeConstraints:self.pageControlVConstraints];
        NSArray * pageControlVConstraints = [NSLayoutConstraint constraintsWithVisualFormat:[NSString stringWithFormat:@"V:[pageControl(10)]-(%f)-|",self.pageControlBottom] options:0 metrics:nil views:@{@"pageControl":self.pageControl}];
        [self addConstraints:pageControlVConstraints];
        self.pageControlVConstraints = pageControlVConstraints;
    }
}
...
- (void)setImageURLStringsGroup:(NSArray *)imageURLStringsGroup
{
    ...
    if (imageURLStringsGroup.count==1&&self.hidesForSinglePage) {
